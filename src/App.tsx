...
export default function App() {
  ...
  const [logs, setLogs] = useState<string[]>([
    "Welcome to BantayBill. Connected securely to Firestore.",
        ...
        cancelProfileSub = subscribeToProfile(user.uid, async (profile) => {
          if (profile) {
            ...
            const initialProfile: UserProfile = {
              ...
              email: user.email || 'guest@bantaybill.com',
              currency: 'PHP',
              billingCycleStart: '1',
              twoFactorAuth: false,
              notifyDue: true,
  ...
  const triggerCallAI = async () => {
    ...
    try {
      ...
      setAiTip(`BantayBill Smart Suggestion: Your current water utilities tracking has settled nicely. By bundling internet plan with your dynamic rent cycle, you can save ₱350/mo. Avoid premium streaming redundancies.`);
    } finally {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `bantaybill_export_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8F9FB]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-slate-500 font-medium animate-pulse">Loading BantayBill...</p>
        </div>
      </div>
    );
  }
          ...
          <nav className="sticky top-0 z-50 w-full bg-[#f6fbf5]/80 backdrop-blur-md border-b border-[#bdc9c1] h-16 flex items-center">
            <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-2xl text-[#005d42] tracking-tight font-dashboard-title">BantayBill</span>
              </div>
          ...
          <footer className="bg-[#181d1a] text-[#dfe4df]/60 py-12 px-6">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-xl text-white">BantayBill</span>
              </div>
              <p className="text-xs">&copy; 2026 BantayBill. Premium Bill Management.</p>
            </div>
          </footer>
        </div>
      )}
        ...
        <div className="min-h-screen flex items-center justify-center p-6 bg-[#f0f5f0]">
          <div className="w-full max-w-md bg-white border border-[#bdc9c1] rounded-xl p-8 shadow-sm">
            <header className="flex flex-col items-center mb-6 text-center">
              ...
              <h1 className="font-bold text-3xl font-dashboard-title text-[#005d42] tracking-tight">BantayBill</h1>
        ...
        <div className="h-screen flex overflow-hidden">
          ...
          <nav className={`h-screen w-64 bg-white border-r border-[#bdc9c1] flex flex-col py-6 px-4 z-40 fixed left-0 top-0 transition-transform duration-300 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:block'}`}>
            <div className="mb-8 px-2 flex items-center gap-3">
              ...
              <div>
                <h1 className="font-semibold text-xl text-[#005d42] font-dashboard-title leading-none">BantayBill</h1>
          ...
          <div className="flex-1 flex flex-col md:pl-64 w-full h-screen overflow-hidden">
            ...
            <header className="h-16 w-full px-6 flex items-center justify-between bg-white/80 backdrop-blur-md border-b border-[#ebefea] shrink-0 sticky top-0 z-30">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-lg font-bold font-dashboard-title text-primary block md:hidden">BantayBill</h1>
